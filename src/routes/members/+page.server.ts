...
export const load: PageServerLoad = async ({ url, locals }) => {
	...
	let mine: string[] = [];
	if (locals.user) {
		const { data: f } = await locals.supabase.from('follows').select('following_id').eq('follower_id', locals.user.id);
		mine = (f ?? []).map((r) => r.following_id as string);
	}
	return { q: q ?? '', members: (data as Record<string, unknown>[] | null) ?? [], mine };
};
